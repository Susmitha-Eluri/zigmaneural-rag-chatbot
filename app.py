...
def search_knowledge_base(query: str, top_k: int = 7):
    ...
    Includes synonym mapping for NovaSoft -> ZigmaNeural.
    Uses whole-word matching and improved density ranking.
    """
    # Clean query (remove prefixes like 'ou:' and punctuation)
    ...
    query_words = set(re.findall(r'\w+', query_clean)) - {
        ...
        "do", "does", "are", "which", "how", "tell", "me", "about", "can", "be", "used", "using", "ways", "should"
    }
    
    # Synonym Mapping
    if "zigmaneural" in query_clean:
        query_words.add("novasoft")
    if "novasoft" in query_clean:
    ...
    for doc in KNOWLEDGE_BASE:
        # Smart Chunking: Split by double newlines (with optional spaces)
        ...
        for seg in raw_segments:
            ...
            for word in query_words:
                if re.search(fr"\b{re.escape(word)}\b", seg_lower):
                    found_words += 1
                    if word in ["zigmaneural", "novasoft", "ai", "agent", "solution", "system"]:
                        hits += 0.5
                    else:
                        hits += 5.0 # High weight for unique words like 'contact', 'manufacturing'
            
            if found_words > 0:
                # Coverage ratio: how many of the query words are in THIS segment?
                coverage_ratio = found_words / len(query_words)
                # Boost segments that have higher coverage
                score = hits * (1 + coverage_ratio ** 2)
                
                # Big bonus for exact phrase matching
